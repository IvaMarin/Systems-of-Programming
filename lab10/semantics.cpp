...
int tSM::p09() { // E -> $id
  ...
  switch (scope) {
    ...
    default: 
      ...
      if (ref.test(BUILT)) {
        ferror_message += "Error[09-1] in line "+ S1->line +": the built-in '"
                          + name +
      ...
      ferror_message += "Error[09-2] in line "+ S1->line +": the name '"
                        + name +
...
int tSM::p16() { // PCPAR -> PCPAR $id
  if (params.count(S2->name)) {
    ferror_message += "Error[16-1] in line "+ S2->line +": the procedure '"
                      + S1->name +
                      "' duplicates \n\t\t\tthe '"
                      + S2->name + "' parameter!\n";
  ...
  if (S2->name == S1->name) {
    ferror_message += "Warning[16-2] in line "+ S2->line +": procedure '"
                      + S1->name +
                      "'has the same name \n"
                      "\t\t\tas its parameter!\n";
