...
// добавляет одну команду (from,c)->to
  ...
  void final(tState st); // включает одно состояние в множество заключительных 
  int  apply(const tSymbol* input); // применяет автомат к входной цепочке                   

  size_t size()const{return table.size();} // выдает размер (кол-во состояний) автомата                                    
...
inline void tFSM::add(tState from,tSymbol c,tState to){
  ...
  table[from][c] = to; // 2 перегруженных оператора []:
                       // 1-й для vector, 2-й для map
}
